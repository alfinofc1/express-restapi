...
async function Instagram(url) {
  let result = "";
  try {
    result = await ig(url); // Memanggil fungsi ig untuk mendapatkan data
  } catch (e) {
    try {
      result = await getDownloadLinks(url); // Jika gagal, coba ambil link download
    } catch (e) {
      result = {
        msg: "Try again later"
      };
  ...
  // Menyesuaikan format hasil agar sesuai dengan format JSON yang diinginkan
  return {
    Creator: "gopalasu",  // Nama Creator
    Data: {
      username: result.metadata?.username || "No title",  // Mengambil caption atau teks dari metadata
      url: result.url || [],  // URL video atau gambar
    },
  };
}
